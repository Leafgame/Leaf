		...
		else if (text != null)
		{
			...
			for (int i = 0; i < textLength; ++i)
			{
				...
				else
				{
					...
					v1.x = v0.x + invSize.x * mChar.maxX;
					v1.y = v0.y - invSize.y * mChar.maxY;

				    u0.x = mChar.uvBottomLeft.x;
					u0.y = mChar.uvBottomRight.y;
					u1.x = mChar.uvTopRight.x;
					u1.y = mChar.uvTopLeft.y;

					x += mSpacingX + (int)mChar.advance;
